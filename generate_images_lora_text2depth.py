...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=True,
        help="cfg1: baseline depth-LoRA; cfg2: config 2 UNet-LoRA; cfg3: config 3 UNet-LoRA. "
             "Text encoder is shared across configs at inference time.",
    )
    parser.add_argument(
        "--predictor_type",
        type=str,
...
def load_text_encoder(
    ...
    """
    Always use the same base CLIP text encoder from the SD2.1 model
    for all configs (cfg1, cfg2, cfg3), so that prompts are encoded
    in a consistent way across configurations.
    """
